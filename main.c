...

{
	...
	*csr_vals = (double *)malloc(nnz * sizeof(double));
	...
	int i, final;
	for (i = 0; i < nnz; i++){
		(*csr_row_ptr)[row_ind[i]]++;//increments how many elements are at row_ind[i]
		row_ptr[row_ind[i]]++;//keeps track of count of each row
	}
	for (i = 1; i < m + 1; i++){
		(*csr_row_ptr)[i] += (*csr_row_ptr)[i - 1];//finalizes row_ptr by putting start of indices in proper places
		row_ptr[i] += row_ptr[i - 1];//finalizes row pointers lipe csr_row_ptr

	}
	for (i = 0; i < nnz; i++){
		final = row_ptr[row_ind[i] - 1];//gets row count at row_ind
		(*csr_vals)[final] = val[i];//sets val at inputted row
		(*csr_col_ind)[final] = col_ind[i];//sets col at inputted row
		row_ptr[row_ind[i] - 1]++;//increments rowPt at inputted row
	}
	free(row_ptr);
}
...
void spmv(unsigned int* csr_row_ptr, unsigned int* csr_col_ind, 
	...
	for(int i = 0; i < m; i++){
		double total = 0.0;
		unsigned int start = csr_row_ptr[i];//sets start position
		unsigned int fin = csr_row_ptr[i + 1];//sets end
		
		for(unsigned int x = csr_row_ptr[i]; x < csr_row_ptr[i + 1]; x++){
			total += csr_vals[x] * vector_x[csr_col_ind[x] - 1];//multiplies the vector and matrix
		}
		
		res[i] = total;//sets final output
	}
}
...
/* This function stores a vector in a text file, similar in format to
   ...
   input parameters:
       ...
       diouble** res 		Array that contains the vector
